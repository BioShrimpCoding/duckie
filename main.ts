...
namespace lighting {
    ...
    export class LightSource {
        ...
        apply() {
            ...
            screen.fillRect(0, 0, screen.width, cy - halfh2 + 1, 15)
            screen.fillRect(0, cy - halfh2 + 1, cx - halfh2, halfh2 << 1, 15)
            screen.fillRect(cx + halfh2, cy - halfh2 + 1, screen.width - cx - halfh2, halfh2 << 1, 15)
            screen.fillRect(0, cy + halfh2, screen.width, screen.height - (cy + halfh2), 15)

            // Go over each row and darken the colors
            for (let y3 = 0; y3 < halfh2; y3++) {
                band2 = this.rings;
                prev = 0;
                offset = this.offsetTable[y3 * this.rings + band2 - 1]

                // Black out the region outside the darkest light band
                screen.mapRect(cx - halfh2, cy + y3 + 1, halfh2 - offset, 1, bandPalettes[bandPalettes.length - 1])
                screen.mapRect(cx - halfh2, cy - y3, halfh2 - offset, 1, bandPalettes[bandPalettes.length - 1])
                screen.mapRect(cx + offset, cy + y3 + 1, halfh2 - offset, 1, bandPalettes[bandPalettes.length - 1])
                screen.mapRect(cx + offset, cy - y3, halfh2 - offset, 1, bandPalettes[bandPalettes.length - 1])
                ...
                while (band2 > 0) {
                    ...
                    screen.mapRect(cx + offset, cy + y3 + 1, prev - offset, 1, bandPalettes[band2 - 1])
                    screen.mapRect(cx - prev, cy + y3 + 1, prev - offset, 1, bandPalettes[band2 - 1])
                    screen.mapRect(cx + offset, cy - y3, prev - offset, 1, bandPalettes[band2 - 1])
                    screen.mapRect(cx - prev, cy - y3, prev - offset, 1, bandPalettes[band2 - 1])
    ...
    export class LanternEffect implements effects.BackgroundEffect {
        ...
        private constructor() {
            bandPalettes = [];
            for (let band3 = 0; band3 < 6; band3++) {
                const buffer = control.createBuffer(16);
                for (let i = 0; i < 16; i++) {
                    buffer[i] = palette_ramps.getPixel(i, band3 + 1);
                }
